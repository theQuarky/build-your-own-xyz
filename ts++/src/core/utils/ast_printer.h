...
#include "parser/nodes/statement_nodes.h"  // Added for ExpressionStmtNode
...
private:
    ...
    void visitExprStmt(const nodes::ExpressionStmtNode* node) {
        indent();
        std::cout << GREEN << "ExprStmt " << getLocationString(node->getLocation()) << RESET << "\n";
        indentLevel_++;
    ...
    void visitExpr(const nodes::ExpressionNode* expr) {
        indentLevel_++;
        if (auto literal = dynamic_cast<const nodes::LiteralExpressionNode*>(expr)) {
            indent();
            std::cout << YELLOW << "Literal '" << literal->getValue() << "' "
                      << getLocationString(literal->getLocation()) << RESET << "\n";
        } 
        else if (auto binary = dynamic_cast<const nodes::BinaryExpressionNode*>(expr)) {
            indent();
        ...
        else if (auto ident = dynamic_cast<const nodes::IdentifierExpressionNode*>(expr)) {
            indent();
            std::cout << YELLOW << "Identifier '" << ident->getName() << "' "
                      << getLocationString(ident->getLocation()) << RESET << "\n";
        }
        else if (auto assign = dynamic_cast<const nodes::AssignmentExpressionNode*>(expr)) {
            indent();
            std::cout << YELLOW << "AssignExpr ("
                      << tokenTypeToString(assign->getExpressionType()) << ") "
                      << getLocationString(assign->getLocation()) << RESET << "\n";
    ...
    void visitType(const nodes::TypeNode* type) {
        ...
        if (auto arrType = dynamic_cast<const nodes::ArrayTypeNode*>(type)) {
            indentLevel_++;
            indent();
            std::cout << BLUE << "Element Type: " << arrType->getElementType()->toString()
                      << RESET << "\n";
            if (arrType->getSize()) {
                indent();
        ...
        else if (auto ptrType = dynamic_cast<const nodes::PointerTypeNode*>(type)) {
            indentLevel_++;
            indent();
        ...
        else if (auto refType = dynamic_cast<const nodes::ReferenceTypeNode*>(type)) {
            indentLevel_++;
            indent();
            std::cout << BLUE << "Referenced Type: " << refType->getBaseType()->toString()
                      << RESET << "\n";
            indentLevel_--;
        } 
        else if (auto funcType = dynamic_cast<const nodes::FunctionTypeNode*>(type)) {
            indentLevel_++;
            indent();
            std::cout << BLUE << "Return Type: " << funcType->getReturnType()->toString()
                      << RESET << "\n";
            indent();
        ...
        else if (auto tmplType = dynamic_cast<const nodes::TemplateTypeNode*>(type)) {
            indentLevel_++;
            indent();
            std::cout << BLUE << "Base Template: " << tmplType->getBaseType()->toString()
                      << RESET << "\n";
            indent();
        ...
        else if (auto spType = dynamic_cast<const nodes::SmartPointerTypeNode*>(type)) {
            indentLevel_++;
            indent();
            std::cout << BLUE << "Pointee Type: " << spType->getPointeeType()->toString()
                      << RESET << "\n";
            indentLevel_--;
        } 
        else if (auto unionType = dynamic_cast<const nodes::UnionTypeNode*>(type)) {
            indentLevel_++;
            indent();
...
public:
    void print(const parser::AST& ast) {
        ...
        for (const auto& node : nodes) {
            ...
            else if (auto exprStmt = std::dynamic_pointer_cast<nodes::ExpressionStmtNode>(node)) {
